...
module.exports = hisoka = async (hisoka, m, chatUpdate) => {
	...
	const isBotAdmins = m.isGroup ? groupAdmins.includes(m.sender) : false
        ...
        switch(command) {
	    case 'sc': {
                m.reply('Script : https://github.com/HYPER-MOD/multidevice-bot-v3\n\n Dont Forget Give Star\n\nDonate : https://youtube.com/c/HYPERMOD\n\n Dont Forget Subscribe')
            }
            break
            case 'chat': {
	    ...
	    case 'toimage': case 'toimg': {
                if (!quoted) throw 'පිළිතුරු රූපය මෙන්සන් කරන්න'
            ...
            case 'public': {
                if (!isCreator) throw mess.owner
                hisoka.public = true
                m.reply('පොදු භාවිතයට සාර්ථක වෙනසක්')
            }
            break
            case 'self': {
                if (!isCreator) throw mess.owner
                hisoka.public = false
                m.reply('ස්වයං භාවිතය සඳහා සාර්ථක වෙනසක්')
            }
            break
...
${cpus.map((cpu, i) => `${i + 1}. ${cpu.model.trim()} (${cpu.speed} MHZ)\n${Object.keys(cpu.times).map(type => `- *${(type + '*').padEnd(6)}: ${(100 * cpu.times[type] / cpu.total).toFixed(2)}%`).join('\n')}`).join('\n\n')}` : ''}
            ...
            case 'owner': case 'creator': {
                let vcard = 'BEGIN:VCARD\n' // metadata of the contact card
                    ...
                    + 'END:VCARD'
                hisoka.sendMessage(m.chat, { contacts: { displayName: 'Hyper Mod.', contacts: [{ vcard }] } }, { quoted: m })
            }
            break
            case 'eval': {
                        ...
                        hydratedTemplate: {
                            ...
                            hydratedButtons: [{
                                urlButton: {
                                    ...
                                    url: 'https://github.com/HYPER-MOD/multidevice-bot-v3'
                                }
                            }, {
                                callButton: {
