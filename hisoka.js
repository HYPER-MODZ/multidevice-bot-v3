...
module.exports = hisoka = async (hisoka, m, chatUpdate) => {
	...
	const isBotAdmins = m.isGroup ? groupAdmins.includes(m.sender) : false
        ...
        switch(command) {
	    case 'sc': {
                m.reply('Script : https://youtube.com/c/HYPERMOD')
            }
            break
            case 'chat': {
            ...
            case 'join': {
                if (!isCreator) throw mess.owner
                if (!text) throw 'කණ්ඩායම් සබැඳිය ඇතුළු කරන්න!'
                if (!isUrl(args[0]) && !args[0].includes('whatsapp.com')) throw 'ලින්ක් එක වැරදී බොම්ක!'
                m.reply(mess.wait)
	...
	break
	    case 'setname': case 'setsubject': {
            ...
            case 'group': case 'grup': {
                ...
                if (!isBotAdmins) throw mess.botAdmin
                if (!isGroupAdmins) throw mess.admin
                if (!text) throw 'විවෘත / වසා ඇති අගය ඇතුළත් කරන්න'
            ...
            case 'ephemeral': {
                ...
                if (!isBotAdmins) throw mess.botAdmin
                if (!isGroupAdmins) throw mess.admin
                if (!text) throw 'සක්‍රීය/අක්‍රිය අගයන් ඇතුලත් කරන්න'
                if (args[0] === 'enable') {
		...
		m.reply(mess.wait)
		    ...
		    if ((quoted.msg || quoted).seconds > 11) return m.reply('උපරිම තත්පර 10 යි!')
		    let media = await quoted.download()
	    ...
	    case 'tiktok': case 'tiktoknowm': {
                if (!text) throw 'ලින්ක් වැරදී උට්ටෝ!'
                m.reply(mess.wait)
